#' Compute the Component Value at Risk of a Portfolio
#'
#' Using the Asset Normal Approach, the function computes the Component Value at Risk of a portfolio for a given time series of returns and a vector of monetary positions.
#' It reflects the contribution of each asset to the whole Portfolio VaR.
#'
...
#'
#' @return The function returns a class "matrix" object containing the Component VaR of every asset of the portfolio for its pre-specified monetary exposure.
...
#' ComponentVaR <- Portfolio_Component_VaR(Returns, 0.99, Wealth, 10)
#' ## End (Not run)
#' @export
Portfolio_Component_VaR <- function (return, confidence_level, wealth, T) {
  ...
  rownames(CVaR) <- paste0(new_column_name)
  colnames(CVaR) <- paste0("Component VaR (", confidence_level, ", ", T, ")")
  return(CVaR)
  assign("output", CVaR, envir=globalenv())
}
